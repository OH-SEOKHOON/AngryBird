...
public class black_bird : MonoBehaviour
{
    public AudioClip dead;
    public AudioClip exp;
    AudioSource aud;

    private bool exptrigger = false;
    ...
    public GameObject explosionRange; // 감지용 자식 오브젝트
    ...
    {
        animator.enabled = true;
        this.animator.SetTrigger("BoomTrigger");
        yield return null;
    }
    ...
    {
        // 폭발 범위 내의 모든 객체 감지
        Collider2D[] colliders = Physics2D.OverlapCircleAll(explosionRange.transform.position, explosionRange.GetComponent<CircleCollider2D>().radius);
